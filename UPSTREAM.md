# UPSTREAM — eventcatalog-slack-bot
...
```bash
git clone https://github.com/pangin/eventcatalog-slack-bot.git && cd eventcatalog-slack-bot
git remote add upstream https://github.com/event-catalog/eventcatalog-slack-bot.git
...
- root: 사유(Proprietary) — LICENSE 파일 없음, README: 'Copyright EventCatalog Ltd. All rights reserved. Usage requires a valid EventCatalog Scale license'
- 상용/제한 경로: `저장소 전체`
- production 제한: Scale license 없이는 실행 불가
- license key 환경변수: `EVENTCATALOG_SCALE_LICENSE_KEY`
- 라이선스 파일: 없음
- upstream에서 위 파일이 바뀌면 sync workflow가 자동 반영을 중단하고 owner 검토를 요구합니다.

## upstream workflow가 fork에서 갖는 위험

- ci.yml만 존재(push/PR to main), secret 불필요
